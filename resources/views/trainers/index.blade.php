				...
				<tr>
					...
					<td>
						<div class="d-flex gap-2">
							<div class="d-flex gap-2">
								<button class="btn btn-icon btn-primary btn-edit" data-id="{{ $trainer->id }}">
									<i class="ti ti-pencil icon"></i>
								</button>
								<button class="btn btn-icon btn-danger btn-delete" data-id="{{ $trainer->id }}">
...
<script>
	...
	$(document).on('click', '.btn-edit', function(){
		...
		$.ajax({
			url: '{{ route('trainers.index') }}' + '/' + id + '/edit/',
			method: 'GET',
			success: function(data){
