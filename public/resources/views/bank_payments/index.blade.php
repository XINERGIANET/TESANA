...
<div class="card">
	...
	<div class="table-responsive">
		<table class="table card-table table-vcenter">
			...
			<tbody>
				@if($bank_payments->count() > 0)
				@foreach($bank_payments as $bank_payment)
				<tr>
					<td>{{ $bank_payment->amount }}</td>
					<td>{{ $bank_payment->bank }}</td>
					<td>{{ $bank_payment->quotas }}</td>
					<td>{{ $bank_payment->date->format('d/m/Y') }}</td>
					<td>
						<div class="d-flex gap-2">
							<button class="btn btn-icon btn-primary btn-edit " data-id="{{ $bank_payment->id }}">
								<i class="ti ti-pencil icon"></i>
							</button>
							<button class="btn btn-icon btn-danger btn-delete" data-id="{{ $bank_payment->id }}">
	...
	@if($bank_payments->hasPages())
	<div class="card-footer d-flex align-items-center">
		{{ $bank_payments->withQueryString()->links() }}
	</div>
	@endif
</div>
  		...
  		<form id="storeForm" method="POST">
  			...
  			<div class="modal-body">
  			  <div class="row">
  			  	...
  			  	<div class="col-lg-6">
  			  		<div class="mb-3">
  			  			<label class="form-label required">Cuotas</label>
  			...
  			<div class="modal-body">
  			  <div class="row">
  			  	...
  			  	<div class="col-lg-6">
  			  		<div class="mb-3">
  			  			<label class="form-label required">Banco</label>
  			  			<input type="text" class="form-control" name="bank" id="editBank" autocomplete="off">
  			  		</div>
  			  	</div>
  			  	<div class="col-lg-6">
  			  		<div class="mb-3">
  			  			<label class="form-label required">Cuotas</label>
  			  			<input type="text" class="form-control" name="quotas" id="editQuotas" autocomplete="off">
  			  		</div>
  			  	</div>
  			  	<div class="col-lg-6">
  			  		<div class="mb-3">
...
<script>
	...
	$(document).on('click', '.btn-edit', function(){
		...
		$.ajax({
			url: '{{ route('bank-payments.index') }}' + '/' + id + '/edit/',
			method: 'GET',
			success: function(data){
				$('#editAmount').val(data.amount);
				$('#editBank').val(data.bank);
				$('#editQuotas').val(data.quotas);
	...
	$('#editForm').submit(function(e){
		...
		$.ajax({
			url: '{{ route('bank-payments.index') }}' + '/' + id + '',
			method: 'PATCH',
			data: $(this).serialize(),
			success: function(data){
				if(data.status){
